...
namespace Pathfinding {
	...
	public class Seeker : VersionedMonoBehaviour {
		[Header("在ONGUI 方法中添加了连线 和刷新path")]
		[Space(10)]
		[Tooltip("线条材质")]
		...
		private void OnGUI() {
			if (GUI.Button(new Rect(0,0,100,40),"line"))
			{
				// add line renderer
				...
				if (lastCompletedNodePath!=null)
				{
					lineRenderer.positionCount=lastCompletedNodePath.Count;
				lineRenderer.SetPosition(0,transform.position);
				lineRenderer.SetPosition(1,(Vector3)lastCompletedNodePath[1].position);
				lineRenderer.SetPosition(2,(Vector3)lastCompletedNodePath[2].position);
					for (int i = 3; i < lastCompletedNodePath.Count; i++)
					{
						Vector3 pos=(Vector3)lastCompletedNodePath[i].position;
					...
					// NavGraph []   graphsToScan=lastCompletedNodePath.ToArray();
